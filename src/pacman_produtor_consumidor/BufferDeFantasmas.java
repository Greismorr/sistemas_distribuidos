...
public class BufferDeFantasmas extends UnicastRemoteObject implements BufferInterface{
    ...
    private int[] vetorBuffer = new int[Manipulador.getTamanhoBuffer()];
    ...
    public synchronized int comer() throws RemoteException {
        ...
        while (contador == 0 || contador < Manipulador.getTamanhoBuffer()) {
            try {
				wait();
        ...
        filaSaida = (filaSaida + 1) % Manipulador.getTamanhoBuffer();
        contador = contador - 1;
        notifyAll();
    ...
    public synchronized void criarFantasma() throws RemoteException {
        ...
        while (contador == Manipulador.getTamanhoBuffer()) {
            try {
                wait();
            } catch (InterruptedException e) {
        ...
        filaEntrada = (filaEntrada + 1) % Manipulador.getTamanhoBuffer();
